...
1. Create Java class implenting javax.servlet.Filter i/f
...
Invoked by WC -- per every request(pre) & resp(post) processing time.

Here you can do pre-processing of req, then invoke chain.doFilter -- to invoke next component of filter chain --- finally it invokes service method of JSP/Servlet --- on its ret path visits filter chain in opposite manner & finally renders response to clnt browser.
